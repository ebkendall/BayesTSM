...
labels <- c('baseline(x)', 'betaZ1(x)', 'betaZ2(x)',
            'baseline(t)', 'betaZ1(t)', 'betaZ2(t)')
...
for(seed in index_seeds){
	...
	if(simulation) {
	    if(no_s2_s3) {
	        file_name = paste0('Model_out/mcmc_out_',toString(seed), '_', trial_num, '_no.rda')
	    } else {
	        file_name = paste0('Model_out/mcmc_out_',toString(seed), '_', trial_num, '_yes.rda')
	    }
	} else {
	    file_name = paste0('Model_out/mcmc_out_',toString(seed), '_', trial_num, '_btsm.rda')
	}
...
if(simulation) {
    if(no_s2_s3) {
        plot_title = paste0('Plots/mcmc_out_', trial_num, '_no.pdf')
    } else {
        plot_title = paste0('Plots/mcmc_out_', trial_num, '_yes.pdf')
    }
} else {
